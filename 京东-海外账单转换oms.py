...
def combine_bill():
    ...
    table["currency_cny_rate"] = table.apply(
        lambda x: x["currency_cny_rate"] if x["currency_cny_rate"] != 0 else x["rate"], axis=1)
    table["currency_cny_rate"] = table.apply(
        lambda x: x["rate1"] if ((x["currency_cny_rate"] == 0) & (x["rate"] == 0)) else x["currency_cny_rate"], axis=1)
    table["currency_cny_rate"] = table.apply(
        lambda x: x["currency_cny_rate"] if x["currency_cny_rate"] > 0 else x["rate2"], axis=1)
    table["INCOME_AMOUNT"] = table.apply(
        lambda x: x["overseas_income"] * x["currency_cny_rate"] if x["INCOME_AMOUNT"] == 0 else x["INCOME_AMOUNT"],
        axis=1)
    table["EXPEND_AMOUNT"] = table.apply(
        lambda x: x["overseas_expend"] * x["currency_cny_rate"] if x["EXPEND_AMOUNT"] == 0 else x["EXPEND_AMOUNT"],
        axis=1)
    table["CREATED"] = table["CREATED"].astype("datetime64[ns]")
