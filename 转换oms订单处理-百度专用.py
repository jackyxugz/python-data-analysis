...
def read_excel(filename):
    print(filename)
    if filename.find("百度") > 0:
        df = pd.read_excel(filename, skiprows=9, dtype=str)
        ...
        # df["运费"] = df["运费"].astype(float)
        # df["优惠金额"] = df["优惠金额"].astype(float)
        ...
        df1["MODIFIED"] = ""
        df1["PAY_TIME"] = ""
        df1["POST_FEE"] = df["运费"].astype(float)
        df1["DISCOUNT_FEE"] = df["优惠金额"].astype(float)
        df1["STATUS"] = df["订单状态"].apply(lambda x:get_status(x))
        df1["CREATED"] = df["下单时间"].str.replace("年","-").str.replace("月","-").str.replace("日","").str.replace("时",":").str.replace("分",":").str.replace("秒","")
        # df1["TID"] = df["订单编号"]
        if "第三方订单ID" in df.columns:
            df["第三方订单ID"] = df["第三方订单ID"].astype(str)
        else:
            df["第三方订单ID"] = "nan"
        df1["TID"] = df.apply(lambda x: x["订单编号"] if x["第三方订单ID"].find("nan") >= 0 else x["第三方订单ID"], axis=1)
        df1["RECEIVED_PAYMENT"] = df["实付金额"]
        df1["BUYER_NICK"] = ""
        df1["SELLER_NICK"] = ""
        df1["SELLER_MEMO"] = df["商户备注"]
        if "发货时间" in df.columns:
            df1["CONSIGN_TIME"] = df["发货时间"].str.replace("年","-").str.replace("月","-").str.replace("日","").str.replace("时",":").str.replace("分",":").str.replace("秒","")
        else:
            df1["CONSIGN_TIME"] = ""
        if "订单完成时间" in df.columns:
            df1["END_TIME"] = df["订单完成时间"].str.replace("年","-").str.replace("月","-").str.replace("日","").str.replace("时",":").str.replace("分",":").str.replace("秒","")
        else:
            df1["END_TIME"] = ""
        df1["PLATFORM"] = plat
        df1["SHOPCODE"] = ""
        if "用户留言" in df.columns:
            df1["BUYER_MESSAGE"] = df["用户留言"]
        else:
            df1["BUYER_MESSAGE"] = ""
        df1["PAY_NO"] = ""
        if "支付方式" in df.columns:
            df1["PAY_WAY"] = df["支付方式"].apply(lambda x:get_payway(x))
        else:
            df1["PAY_WAY"] = ""
        if "售后状态" in df.columns:
            df1["REFUND_STATUS"] = df["售后状态"].apply(lambda x:get_refund_status(x) if pd.notnull(x) else "")
        else:
            df1["REFUND_STATUS"] = ""
        if "完整地址" in df.columns:
            df1["RECEIVER_ADDRESS"] = df["完整地址"]
            df1["RECEIVER_MOBILE"] = df["收件人电话"]
            df1["RECEIVER_NAME"] = df["收件人"]
            df1["RECEIVER_STATE"] = df["省"]
            df1["RECEIVER_ZIP"] = ""
            df1["RECEIVER_DISTRICT"] = df["区"]
            df1["RECEIVER_CITY"] = df["市"]
            df1["RECEIVER_TOWN"] = df["街道"]
        elif "收货地址" in df.columns:
            df1["RECEIVER_ADDRESS"] = df["收货地址"]
            df1["RECEIVER_MOBILE"] = df["下单手机号"]
            df1["RECEIVER_NAME"] = df["买家姓名"]
            df1["RECEIVER_STATE"] = ""
            df1["RECEIVER_ZIP"] = ""
            df1["RECEIVER_DISTRICT"] = ""
            df1["RECEIVER_CITY"] = ""
            df1["RECEIVER_TOWN"] = ""
        else:
            df1["RECEIVER_ADDRESS"] = ""
            df1["RECEIVER_MOBILE"] = df["收件人电话"]
            df1["RECEIVER_NAME"] = df["收件人"]
            df1["RECEIVER_STATE"] = df["省"]
            df1["RECEIVER_ZIP"] = ""
        ...
        # df1["SELLER_NICK"] = df["商品ID"].apply(lambda x: get_baidushop(x, filename, 1, shop_df))
        # df1["SHOPCODE"] = df1["SELLER_NICK"].apply(lambda x: get_baidushop(x, filename, 2, shop_df))
        if filename.find("SF萌齿洁11")>=0:
            df1["SELLER_NICK"] = "SF萌齿洁11"
            df1["SHOPCODE"] = "hzzt11"
        elif filename.find("SF萌齿洁13")>=0:
            df1["SELLER_NICK"] = "SF萌齿洁13"
            df1["SHOPCODE"] = "hzzt13"
        elif filename.find("SF萌齿洁15")>=0:
            df1["SELLER_NICK"] = "SF萌齿洁15"
            df1["SHOPCODE"] = "hzzt15"
        elif filename.find("SF萌齿洁17")>=0:
            df1["SELLER_NICK"] = "SF萌齿洁17"
            df1["SHOPCODE"] = "hzzt17"
        elif filename.find("SF萌齿洁18")>=0:
            df1["SELLER_NICK"] = "SF萌齿洁18"
            df1["SHOPCODE"] = "hzzt18"
        elif filename.find("深分特殊化妆品-麦凯莱1")>=0:
            df1["SELLER_NICK"] = "深分特殊化妆品-麦凯莱1"
            df1["SHOPCODE"] = "mklzt_1"
        elif filename.find("深分特殊化妆品-麦凯莱3")>=0:
            df1["SELLER_NICK"] = "深分特殊化妆品-麦凯莱3"
            df1["SHOPCODE"] = "mklzt_3"
        elif filename.find("深分特殊化妆品-麦凯莱4")>=0:
            df1["SELLER_NICK"] = "深分特殊化妆品-麦凯莱4"
            df1["SHOPCODE"] = "mklzt_4"
        elif filename.find("深分特殊化妆品-麦凯莱")>=0:
            df1["SELLER_NICK"] = "深分特殊化妆品-麦凯莱"
            df1["SHOPCODE"] = "mklzt_0"
        elif filename.find("SF麦凯莱科技04")>=0:
            df1["SELLER_NICK"] = "SF麦凯莱科技04"
            df1["SHOPCODE"] = "mklzt4"
        elif filename.find("SF麦凯莱科技")>=0:
            df1["SELLER_NICK"] = "SF麦凯莱科技"
            df1["SHOPCODE"] = "mklzt0"

        elif filename.find("广分电商-尚西10")>=0:
            df1["SELLER_NICK"] = "广分电商-尚西10"
            df1["SHOPCODE"] = "sxzt10"
        ...
        df2 = pd.DataFrame()
        if "选购产品" in df.columns:
            df2["TITLE"] = df["选购产品"]
        else:
            df2["TITLE"] = df["商品名称"]
        if "sku编码" in df.columns:
            df2["SKU"] = df["sku编码"]
        elif "商家编码" in df.columns:
            df2["SKU"] = df["商家编码"]
        elif "商品编码" in df.columns:
            df2["SKU"] = df["商品编码"]
        else:
            df2["SKU"] = ""
        df2["PRICE"] = df1["PAYMENT"]/df["数量"].astype(int)
        df2["PAYMENT"] = df1["PAYMENT"]
        df2["PIC_PATH"] = ""
        df2["NUM"] = df["数量"].astype(int)
        df2["PAY_TIME"] = ""
        if "快递公司" in df.columns:
            df2["LOGISTICS_COMPANY"] = df["快递公司"]
            df2["INVOICE_NO"] = df["单号"]
        else:
            df2["LOGISTICS_COMPANY"] = df["快递名称"]
            df2["INVOICE_NO"] = df["快递单号"]
        # df2["TID"] = df["订单编号"]
        df2["TID"] = df.apply(lambda x: x["订单编号"] if x["第三方订单ID"].find("nan") >= 0 else x["第三方订单ID"], axis=1)
        df2["SKU_PROPERTIES_NAME"] = ""
        df2["OID"] = ""
        if "售后状态" in df.columns:
            df2["REFUND_STATUS"] = df["售后状态"].apply(lambda x:get_refund_status(x) if pd.notnull(x) else "")
            df2["CONSIGN_TIME"] = df["发货时间"].str.replace("年","-").str.replace("月","-").str.replace("日","").str.replace("时",":").str.replace("分",":").str.replace("秒","")
        else:
            df2["REFUND_STATUS"] = ""
            df2["CONSIGN_TIME"] = ""
...
def get_status(status):
    ...
    elif ((status.find("交易关闭") >= 0) | (status.find("删除") >= 0) | (status.find("取消") >= 0) | (status.find("在线支付超时") >= 0) | (status.find("订单取消") >= 0)):
        return "TRADE_CLOSED"
...
def combine_excel():
    ...
    if type == 6:
        file = open_file()
        default_dir = (os.sep).join(file.split(os.sep)[:-1])
        table1, table2 = read_excel(file)
    else:
        try:
    ...
    table1["TID"] = table1["TID"].str.replace("\s+","")
