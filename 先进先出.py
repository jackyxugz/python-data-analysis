...
month_list = ["202009", "202010", "202011", "202012", "202101", "202102", "202103", "202104", "202105", "202106",
...
def read_sales():
    ...
    df = df.groupby(["代销月份", "商品号"]).agg({"销售数量": np.sum})
    df = pd.DataFrame(df).reset_index()
    df.columns = ["代销月份", "商品号", "销售数量"]
    return df
...
if __name__ == "__main__":
    ...
    # print(df3)
    print(df3.head(10).to_markdown())
    df3[["sku","yearmonth","求和项:供货总额（不含税）","求和项:含税总额","当月入库","qty_in1","qty_in2","qty_in3","qty_in4","qty_in5","qty_in6","当月出库","qty_out1","qty_out2","qty_out3","qty_out4","qty_out5","qty_out6","累计入库","累计出库","期末库存"]].to_excel("work/自然先进先出.xlsx")
